...
TEMPLATE = {
    "package": "conditional_detr_detection_pipeline",
    "repo_name": "conditional-detr-detection-pipeline",
    "stem": "conditional_detr_detection",
    "notebook_name": "conditional_detr_detection_colab.ipynb",
    "profile": "E2E",
    "mode": "GUIDED",
    "pipeline_class": "ConditionalDetrDetectionPipeline",
    "weights_key": "conditional-detr-resnet-50",
    "modules": [
        "samples.py",
        "pipeline.py",
    ],
    ...
    "title": "Conditional DETR ResNet-50 (COCO) — DIMER object detection and bounded detection fine-tuning (standalone)",
    "badges": [
        (
            "GitHub",
            "https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white",
            "https://github.com/kurtvalcorza/conditional-detr-detection-pipeline",
        ),
        (
            "Open In Colab",
            "https://colab.research.google.com/assets/colab-badge.svg",
            "https://colab.research.google.com/github/kurtvalcorza/conditional-detr-detection-pipeline/blob/main/tutorials/conditional_detr_detection_colab.ipynb",
        ),
        (
            "Hugging Face",
            "https://img.shields.io/badge/%F0%9F%A4%97%20Hugging%20Face-microsoft%2Fconditional--detr--resnet--50-ffcc4d?style=flat",
            "https://huggingface.co/microsoft/conditional-detr-resnet-50",
        ),
        (
            "Upstream",
            "https://img.shields.io/badge/Upstream-Atten4Vis%2FConditionalDETR-181717?style=flat&logo=github&logoColor=white",
            "https://github.com/Atten4Vis/ConditionalDETR",
        ),
        ("arXiv", "https://img.shields.io/badge/arXiv-2108.06152-b31b1b.svg", "https://arxiv.org/abs/2108.06152"),
        ("License", "https://img.shields.io/badge/License-Apache--2.0-green.svg", "https://github.com/kurtvalcorza/conditional-detr-detection-pipeline/blob/main/LICENSE"),
    ],
    "capability": "object detection over the COCO classes with Conditional DETR ResNet-50, a DETR variant built to converge faster, and a bounded detection fine-tune that re-heads it onto your own class vocabulary, evaluates it against a held-out split with COCO-style average precision, and exports a reloadable SafeTensors adapter",
    "intro": (
        "Conditional DETR with a ResNet-50 backbone (`microsoft/conditional-detr-resnet-50`) keeps DETR's design — a ResNet-50 feature map, a "
        "6-layer transformer encoder, a 6-layer decoder and set prediction with Hungarian matching — and changes the decoder's cross-attention. "
        "Each of the 300 queries learns a **conditional spatial query** from its own decoder embedding, so each attention head can look at one "
        "band of the image, such as one edge of the object, instead of relying on content alone. The paper reports that this makes training "
        "converge 6.7× faster than DETR for the R50 backbone. The class head is trained with a **sigmoid focal loss**: every (query, class) pair "
        "has its own independent score, and there is no \"no object\" class. At inference the processor resizes the image so its shorter side "
        "is 800 px, and the top-scoring (query, class) pairs at or above a caller-owned threshold are returned as xyxy boxes in input pixels.\n\n"
        "**The default path really adapts the model:** it re-heads Conditional DETR onto a three-class traffic sign vocabulary that does not exist in "
    ...
    "learning_objectives": (
        ...
        "baseline; run a bounded fine-tune with Conditional DETR's own set-prediction loss (Hungarian matching, sigmoid focal loss, L1 and generalised IoU); "
    ...
    "cells": [
        # ---------------------------------------------------------------- 4. COCO scene
        {
            "md": (
                ...
                "The detection threshold is a **caller-owned request parameter**, not a pipeline constant. Each score is an **independent per-class "
                "sigmoid under the model's focal-loss head, not a calibrated** probability for your images, and one query can appear under two labels. "
                "The default `0.7` is the value the pinned model README uses, and it is passed explicitly on every call.\n\n"
        ...
        # ---------------------------------------------------------------- 7. Split & Baseline
        {
            "md": (
                ...
                "(`class_labels_classifier`, now 3 sigmoid outputs) with a freshly initialised layer. Its bias is set to $-\\log((1-p)/p) \\approx -4.595$ "
                "for a prior $p = 0.01$, so every new score starts near 0.01 instead of 0.5 and the fresh head does not flood the output with boxes. "
                ...
                "this number is the floor the fine-tune has to beat, not a property of Conditional DETR."
            ),
            "code": (
                ...
                "adapter = ConditionalDetrDetectionPipeline.from_pretrained(weights_dir=WEIGHTS_DIR, class_names=SIGN_CLASSES, seed=SEED)\n"
        ...
        # ---------------------------------------------------------------- 8. Bounded Fine-Tuning
        {
            "md": (
                ...
                "This cell runs the real adaptation step in this runtime. It is gradient fine-tuning with Conditional DETR's own loss: each image's "
                "300 queries are matched one-to-one to its reference boxes by the Hungarian algorithm, and the class scores of all queries are trained "
                "with a sigmoid focal loss (`focal_alpha` 0.25) that pushes unmatched queries toward zero, plus an L1 box term and a generalised-IoU "
                "box term on the matched queries.\n\n"
                "- **The backbone is frozen.** Parameters under `model.backbone.` keep their COCO values; the transformer, the class head and the "
                "box head are trained. The cell prints the trainable and total parameter counts.\n"
        ...
        # ---------------------------------------------------------------- 11. Export, reload & verify
        {
            "md": (
                ...
                "`save_artifact` writes `outputs/conditional_detr_adapter.safetensors`: every tensor the fine-tune could change, plus a metadata header naming "
            ...
            "code": (
                "artifact_path = OUTPUTS / 'conditional_detr_adapter.safetensors'\n"
                "descriptor = adapter.save_artifact(artifact_path, notes='Conditional DETR ResNet-50 sign adaptation tutorial adapter')\n"
                "print(json.dumps(descriptor, indent=2))\n\n"
                "reloaded = ConditionalDetrDetectionPipeline.load_artifact(artifact_path, weights_dir=WEIGHTS_DIR)\n"
        ...
        # ---------------------------------------------------------------- 12. Outputs & provenance
        {
            "md": (
                ...
                "The cell writes:\n"
                "- `outputs/conditional_detr_detection_input_manifest.json`\n"
                "- `outputs/conditional_detr_detection_evaluation_report.json`\n"
                "- `outputs/conditional_detr_detection_result.json` (identity, runtime versions, device, dataset manifest, split, baseline and adapted metrics, "
                "fine-tuning configuration, new-data rows, adapter descriptor and reload check)\n"
                "- `outputs/conditional_detr_detection_detections.csv`\n"
                "- `outputs/conditional_detr_detection_annotated.png`\n"
                "- `outputs/conditional_detr_adapter.safetensors` (written in Section 11)"
            ),
            "code": (
                "import csv\n"
        ...
        # ---------------------------------------------------------------- 13. BYOD
        {
            ...
            "code": (
                ...
                "    byod_pipe = ConditionalDetrDetectionPipeline.from_pretrained(weights_dir=WEIGHTS_DIR, class_names=byod_names, seed=SEED)\n"
                ...
                "    byod_descriptor = byod_pipe.save_artifact(OUTPUTS / 'byod_conditional_detr_adapter.safetensors', notes='BYOD adaptation adapter')\n"
                "    ConditionalDetrDetectionPipeline.load_artifact(OUTPUTS / 'byod_conditional_detr_adapter.safetensors', weights_dir=WEIGHTS_DIR)\n"
                "    print('BYOD adapter exported and reloaded:', byod_descriptor['sha256'][:16])\n"
                "else:\n"
                "    print('BYOD dataset branch is off; set USE_BYOD_DATASET = True to adapt Conditional DETR on your own labelled images.')"
            ),
        },
    ],
    "closing": (
        "## Interpretation and limits\n\n"
        "**What this notebook established, in this runtime.** The pinned `microsoft/conditional-detr-resnet-50` snapshot was verified against a committed "
        ...
        "**Try next.** Change `FREEZE_BACKBONE` to `False` and compare held-out AP and runtime, or change `EPOCHS` and watch where held-out AP "
        "stops improving. To transfer the workflow, point `BYOD_DATASET_DIR` at a small labelled set from your own domain.\n\n"
        "## References\n\n"
        "- Meng, D., Chen, X., Fan, Z., Zeng, G., Li, H., Yuan, Y., Sun, L. and Wang, J. (2021). *Conditional DETR for Fast Training Convergence.* [arXiv:2108.06152](https://arxiv.org/abs/2108.06152).\n"
        "- Carion, N. et al. (2020). *End-to-End Object Detection with Transformers.* [arXiv:2005.12872](https://arxiv.org/abs/2005.12872).\n"
        "- Upstream repository: [Atten4Vis/ConditionalDETR](https://github.com/Atten4Vis/ConditionalDETR) — Apache-2.0.\n"
        "- Hugging Face checkpoint: [microsoft/conditional-detr-resnet-50](https://huggingface.co/microsoft/conditional-detr-resnet-50) — Apache-2.0.\n"
        "- Lin, T.-Y. et al. (2014). *Microsoft COCO: Common Objects in Context.* [arXiv:1405.0312](https://arxiv.org/abs/1405.0312).\n"
        "- Repository model card: https://github.com/kurtvalcorza/conditional-detr-detection-pipeline/blob/main/MODEL_CARD.md\n"
        "- [`kurtvalcorza/conditional-detr-detection-pipeline`](https://github.com/kurtvalcorza/conditional-detr-detection-pipeline) — source repository for this pipeline."
    ),
}
