...
def test_pin_writes_commit_digests_and_module_revision(tmp_path):
    ...
    manifest = json.loads((root / "weights/conditional-detr-resnet-50/dimer-base-manifest.json").read_text())
    ...
    module = (root / "src/conditional_detr_detection_pipeline/pipeline.py").read_text()
    assert f'MODEL_REVISION = "{COMMIT}"' in module


def test_pin_refuses_a_digest_that_disagrees_with_the_hub(tmp_path):
    root = _copy_repo(tmp_path)
    before = (root / "weights/conditional-detr-resnet-50/dimer-base-manifest.json").read_text()
    module_before = (root / "src/conditional_detr_detection_pipeline/pipeline.py").read_text()
    ...
    assert (root / "weights/conditional-detr-resnet-50/dimer-base-manifest.json").read_text() == before
    assert (root / "src/conditional_detr_detection_pipeline/pipeline.py").read_text() == module_before
