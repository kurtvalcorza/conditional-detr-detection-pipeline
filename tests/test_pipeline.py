...
from conditional_detr_detection_pipeline import (
    ARTIFACT_FORMAT,
    DEFAULT_WEIGHTS_DIR,
    DETECTION_THRESHOLD,
    LABELS,
...
from conditional_detr_detection_pipeline import pipeline as pipeline_module
...
def test_identity_constants_agree_with_the_committed_manifest():
    ...
    assert MODEL_ID == "microsoft/conditional-detr-resnet-50" == manifest["modelId"]
    ...
    assert ARTIFACT_FORMAT == "conditional-detr-adapter-v1"
...
def test_labels_equal_the_snapshot_id2label_order():
    ...
    assert len(LABELS) == 91 and config["num_queries"] == MAX_DETECTIONS == 300
...
def test_unpinned_package_refuses_every_weight_operation(tmp_path, monkeypatch):
    ...
    for call in (
        ...
        lambda: ConditionalDetrDetectionPipeline.from_pretrained(weights_dir=tmp_path),
    ):
...
def test_verify_snapshot_accepts_matching_manifest(tmp_path, pinned):
    _write_snapshot(tmp_path, pinned, b'{"model_type": "conditional_detr"}')
...
def test_verify_snapshot_rejects_tampered_digest(tmp_path, pinned):
    content = b'{"model_type": "conditional_detr"}'
...
class MockPipeline(ConditionalDetrDetectionPipeline):
...
def test_detect_rejects_malformed_backend_output():
    ...
    flood = MockPipeline(runner=lambda *_: [{"box": [0, 0, 1, 1], "label": "clock", "score": 0.5}] * 301)
