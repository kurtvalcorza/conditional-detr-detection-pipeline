"""End-to-end adaptation on a tiny random-weight Conditional DETR: no checkpoint, no network.

The tiny model has the same classes and code paths as the pinned checkpoint
(ConditionalDetrForObjectDetection with an in-library ResNet backbone instead of the timm one), so these
tests exercise the real fine-tuning loss,
...
from conditional_detr_detection_pipeline import (  # noqa: E402
    ARTIFACT_FORMAT,
    MODEL_ID,
    MODEL_KEY,
    SIGN_CLASSES,
...
from conditional_detr_detection_pipeline import pipeline as pipeline_module  # noqa: E402


def _tiny_pipeline(seed: int = 0, class_names=SIGN_CLASSES) -> ConditionalDetrDetectionPipeline:
    ...
    model = ConditionalDetrForObjectDetection(config).eval()
    processor = ConditionalDetrImageProcessor(size={"shortest_edge": 64, "longest_edge": 64})
    return ConditionalDetrDetectionPipeline(
...
def test_finetune_evaluate_export_and_reload(tmp_path, data):
    ...
    metadata = ConditionalDetrDetectionPipeline.read_artifact_metadata(path)
...
def test_apply_artifact_refuses_mismatched_adapters(tmp_path, data):
    ...
    tensor = {"class_labels_classifier.bias": torch.zeros(3)}
